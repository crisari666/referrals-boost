...
const raw: Array<[string, string, string]> = [
  ...
  ['CI', 'Ivory Coast', '225'], ['JM', 'Jamaica', '1876'], ['JP', 'Japan', '81'],
  ...
  ['CG', 'Republic of the Congo', '242'], ['RE', 'Réunion', '262'], ['RO', 'Romania', '40'],
  ['RU', 'Russia', '7'], ['RW', 'Rwanda', '250'], ['BL', 'Saint Barthélemy', '590'],
  ['SH', 'Saint Helena', '290'], ['KN', 'Saint Kitts and Nevis', '1869'], ['LC', 'Saint Lucia', '1758'],
  ...
  ['SL', 'Sierra Leone', '232'], ['SG', 'Singapore', '65'], ['SX', 'Sint Maarten', '1721'],
  ...
  ['VN', 'Vietnam', '84'], ['WF', 'Wallis and Futuna', '681'], ['YE', 'Yemen', '967'],
...
export const DEFAULT_COUNTRY: CountryCode =
  COUNTRY_CODES.find((c) => c.code === 'CO') ?? COUNTRY_CODES[0];

const DIAL_CODES_BY_LENGTH = [...COUNTRY_CODES].sort(
  (a, b) => b.dial.length - a.dial.length,
);

export function findCountryByDialPrefix(digits: string): {
  country: CountryCode;
  local: string;
} | null {
  ...
  for (const country of DIAL_CODES_BY_LENGTH) {
    if (normalized.startsWith(country.dial)) {
      return {
        country,
        local: normalized.slice(country.dial.length),
      };
    }
  }
  return null;
...
export function splitStoredPhone(stored: string | undefined): {
  ...
  const match = findCountryByDialPrefix(digits);
  if (match) {
    return match;
  }
